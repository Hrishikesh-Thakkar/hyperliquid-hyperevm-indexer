...
export interface MatcherDeps {
  ...
  metrics: { matcherTransfersTotal: Counter };
}

/**
 * Creates a matcher instance with all dependencies injected.
 *
 * The returned object has a single `run()` method that performs one matcher pass:
 * claims eligible pending transfers and attempts to locate their counterpart on HyperEVM.
 */
export function createMatcher(deps: MatcherDeps) {
  const { logger, evmService, transferRepository, metrics } = deps;
  ...
  async function matchTransfer(record: DocumentType<TransferRecord>): Promise<void> {
    ...
    try {
      match =
        record.tokenSymbol === HYPE_SYMBOL || !record.evmTokenAddress
          ? await evmService.findNativeTransfers(record.evmFrom, record.receiver, amountBigInt, afterTimestamp, undefined, excludeSet)
          : await evmService.findErc20Transfers(record.evmTokenAddress, record.evmFrom, record.receiver, amountBigInt, afterTimestamp, undefined, excludeSet);
    } catch (err) {
    ...
    if (match) {
      ...
      metrics.matcherTransfersTotal.inc({ result: 'matched' });
      return;
    }
